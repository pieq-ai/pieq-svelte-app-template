...
export const load: PageServerLoad = async ({ locals }) => {
	...
	if (!employee) {
		...
		if (firstEmployee) {
			...
			const leaveDao = await import('$lib/server/dao/leave.dao.js');
			employment = await leaveDao.getActiveEmploymentByEmployeeCuid(firstEmployee.cuid) ?? null;
		}
	}
	...
	if (subordinatesEmploymentList.length > 0) {
		...
		const subDeptMap = new Map(subDepartments.map((d) => [d.cuid, d.name]));
		...
		managerContext = {
			...
			metrics: {
				...
				regularization: 0, // Default 0 as not modeled in DB
