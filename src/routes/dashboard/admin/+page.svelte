...
<div class="bg-[#FAF9F6] -mx-6 -my-6 p-6 min-h-screen space-y-6">
	...
	<!-- Welcome Header Card -->
	<section class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white border border-neutral-200/80 rounded-2xl p-6 shadow-xs">
		<div class="flex items-center gap-4">
			<div class="size-12 rounded-full bg-[#FFF4EE] flex items-center justify-center border border-[#FFE2D3] text-[#F45310] font-black text-lg">
				A
			</div>
			<div>
				<h2 class="text-xl font-bold text-neutral-900 leading-tight">Welcome back, Admin! 👋</h2>
	...
	<section class="grid grid-cols-2 lg:grid-cols-4 gap-4">
		...
		<div class="bg-white border border-neutral-200/80 rounded-2xl p-4 shadow-xs space-y-2">
			<div class="flex items-center justify-between text-neutral-400">
				<span class="text-[10px] font-extrabold uppercase tracking-wider">Total Employees</span>
				<div class="size-7 rounded-lg bg-indigo-50 flex items-center justify-center text-indigo-600">
					<UsersIcon class="size-3.5 shrink-0" />
				</div>
			</div>
			<div class="text-2xl font-black text-neutral-800">{data.stats?.totalEmployees ?? 0}</div>
		...
		<div class="bg-white border border-neutral-200/80 rounded-2xl p-4 shadow-xs space-y-2">
			<div class="flex items-center justify-between text-neutral-400">
				<span class="text-[10px] font-extrabold uppercase tracking-wider">Departments</span>
				<div class="size-7 rounded-lg bg-blue-50 flex items-center justify-center text-blue-600">
					<Building2Icon class="size-3.5 shrink-0" />
				</div>
			</div>
			<div class="text-2xl font-black text-neutral-800">{data.stats?.departmentsCount ?? 0}</div>
			<div class="flex items-center gap-0.5 text-[10px] font-bold text-neutral-400">
				<span>— No change</span>
			</div>
		</div>
		...
		<div class="bg-white border border-neutral-200/80 rounded-2xl p-4 shadow-xs space-y-2">
			<div class="flex items-center justify-between text-neutral-400">
				<span class="text-[10px] font-extrabold uppercase tracking-wider">Total Payroll (MTD)</span>
				<div class="size-7 rounded-lg bg-amber-50 flex items-center justify-center text-amber-600">
					<WalletIcon class="size-3.5 shrink-0" />
				</div>
			</div>
			<div class="text-base font-black text-neutral-800 truncate" title={formatIndianCurrency(data.stats?.totalPayroll ?? 0)}>
				{formatIndianCurrency(data.stats?.totalPayroll ?? 0)}
			</div>
			<div class={`flex items-center gap-0.5 text-[10px] font-bold ${(data.stats?.totalPayrollTrend ?? 0) >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
	...
	<section class="grid grid-cols-1 lg:grid-cols-3 gap-6">
		...
		<div class="lg:col-span-2 bg-white border border-neutral-200/80 rounded-2xl p-6 shadow-xs flex flex-col justify-start gap-4">
			...
			{#if (data.stats?.totalPayroll ?? 0) === 0}
				<div class="flex flex-col items-center justify-center py-16 text-center">
					<p class="text-sm font-semibold text-neutral-400">No payroll data found for this period.</p>
				</div>
			{:else}
				<div class="flex flex-col md:flex-row items-center justify-around gap-6 py-2">
					...
					<div class="relative flex items-center justify-center size-44 shrink-0">
						...
						<div class="absolute flex flex-col items-center justify-center text-center">
							<span class="text-[9px] font-bold text-neutral-800">{formatAbbreviatedCurrency(data.stats?.totalPayroll ?? 0)}</span>
							<span class="text-[8px] font-bold text-neutral-400 uppercase tracking-wide mt-0.5">Total Payroll</span>
						</div>
					</div>
					...
					<div class="flex-1 space-y-2.5 w-full">
						{#each segments as seg}
							<div class="flex items-center justify-between border-b border-neutral-100 pb-1 text-xs">
		...
		<div class="bg-white border border-neutral-200/80 rounded-2xl p-6 shadow-xs flex flex-col">
			<h3 class="text-base font-bold text-neutral-900 mb-4">Quick Actions</h3>
			
			<div class="grid grid-cols-2 gap-3.5 flex-1">
				<!-- Add Employee -->
				<a href="/employees/create" class="flex flex-col justify-between border border-neutral-150 rounded-xl p-3 hover:bg-[#FFF4EE]/30 hover:border-[#FFE2D3] group transition-all duration-200 decoration-none">
					<div class="size-8 rounded-lg bg-[#FFF4EE] text-[#F45310] flex items-center justify-center">
						<UserPlusIcon class="size-4.5" />
					</div>
					<div class="mt-3">
						<span class="text-xs font-extrabold text-neutral-700 block">Add Employee</span>
						<span class="text-[9px] text-neutral-400 font-medium block mt-0.5 leading-tight">Create new record</span>
					</div>
				</a>
				...
				<!-- Roles & Permissions -->
				<a href="/role-permissions" class="flex flex-col justify-between border border-neutral-150 rounded-xl p-3 hover:bg-indigo-50/30 hover:border-indigo-100 group transition-all duration-200 decoration-none">
					<div class="size-8 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center">
						<ShieldCheckIcon class="size-4.5" />
					</div>
					<div class="mt-3">
						<span class="text-xs font-extrabold text-neutral-700 block">Roles & Perms</span>
						<span class="text-[9px] text-neutral-400 font-medium block mt-0.5 leading-tight">Set permissions</span>
					</div>
				</a>

				<!-- Attendance Overview -->
				<a href="/attendance" class="flex flex-col justify-between border border-neutral-150 rounded-xl p-3 hover:bg-blue-50/30 hover:border-blue-100 group transition-all duration-200 decoration-none">
					<div class="size-8 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center">
						<ClockIcon class="size-4.5" />
					</div>
					<div class="mt-3">
						<span class="text-xs font-extrabold text-neutral-700 block">Attendance</span>
						<span class="text-[9px] text-neutral-400 font-medium block mt-0.5 leading-tight">Overview logs</span>
					</div>
				</a>

				<!-- Payroll -->
				<a href="/payrolls" class="flex flex-col justify-between border border-neutral-150 rounded-xl p-3 hover:bg-amber-50/30 hover:border-amber-100 group transition-all duration-200 decoration-none">
					<div class="size-8 rounded-lg bg-amber-50 text-amber-600 flex items-center justify-center">
						<PlayIcon class="size-4.5 ml-0.5" />
					</div>
					<div class="mt-3">
						<span class="text-xs font-extrabold text-neutral-700 block">Payroll</span>
						<span class="text-[9px] text-neutral-400 font-medium block mt-0.5 leading-tight">Process batches</span>
					</div>
				</a>

				<!-- System Settings -->
				<a href="/settings" class="flex flex-col justify-between border border-neutral-150 rounded-xl p-3 hover:bg-neutral-100/50 group transition-all duration-200 decoration-none">
					<div class="size-8 rounded-lg bg-neutral-100 text-neutral-600 flex items-center justify-center">
						<SettingsIcon class="size-4.5" />
					</div>
					<div class="mt-3">
						<span class="text-xs font-extrabold text-neutral-700 block">Settings</span>
						<span class="text-[9px] text-neutral-400 font-medium block mt-0.5 leading-tight">Global settings</span>
					</div>
				</a>
			</div>
		</div>
