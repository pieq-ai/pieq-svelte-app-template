<script lang="ts">
	...
	// Switcher State
	let activeDashboard = $state('employee');
	...
	$effect(() => {
		...
		// Calculate run-time duration since check-in
...
<svelte:head>
	<title>{activeDashboard === 'employee' ? 'Employee Dashboard' : 'Manager Dashboard'} – PieQ HRMS</title>
</svelte:head>

<!-- Outer background wrapper matching off-white reference style -->
<div class="bg-[#FAF9F6] -mx-6 -my-6 p-6 min-h-screen space-y-6">
		...
		<div class="flex items-center gap-4">
			...
			{#if data.isManager}
				...
				<div class="relative">
					...
					>
						<span>{activeDashboard === 'employee' ? 'Employee Dashboard' : 'Manager Dashboard'}</span>
					...
					{#if isSwitcherOpen}
						...
						<div class="absolute left-0 mt-1 w-full bg-white border border-neutral-200 rounded-xl shadow-lg z-50 py-1">
							<button
								type="button"
								onclick={() => { activeDashboard = 'employee'; isSwitcherOpen = false; }}
							...
							<button
								type="button"
								onclick={() => { activeDashboard = 'manager'; isSwitcherOpen = false; }}
								class="w-full text-left px-4 py-2 text-xs text-neutral-700 hover:bg-neutral-50 cursor-pointer border-none bg-transparent font-bold"
							>
								Manager Dashboard
							</button>
	...
	{#if data.employee}
				...
				<div class="lg:col-span-2 bg-white border border-neutral-200/80 rounded-2xl p-6 shadow-xs flex flex-col justify-between">
					<div>
						<div class="flex items-center justify-between mb-4">
							<div class="space-y-1">
								<h3 class="text-lg font-bold text-neutral-900">Daily Attendance</h3>
							...
							<div class="size-9 rounded-full bg-neutral-100 flex items-center justify-center text-neutral-500">
								<ClockIcon class="size-4.5" />
							</div>
						</div>

						<!-- Attendance Details columns -->
						<div class="grid grid-cols-2 sm:grid-cols-4 gap-6 py-6 border-y border-neutral-100">
							<div>
								<span class="text-xs text-neutral-400 font-semibold uppercase tracking-wider block">Check In</span>
								<span class="text-base font-bold text-neutral-800 mt-1 block">
									{formatTimeOnly(data.todayAttendance?.check_in_time)}
								</span>
							</div>
							<div>
								<span class="text-xs text-neutral-400 font-semibold uppercase tracking-wider block">Check Out</span>
								<span class="text-base font-bold text-neutral-800 mt-1 block">
									{formatTimeOnly(data.todayAttendance?.check_out_time)}
								</span>
							</div>
							<div>
								<span class="text-xs text-neutral-400 font-semibold uppercase tracking-wider block">Work Duration</span>
								<span class="text-base font-bold text-neutral-800 mt-1 block">
									{durationString}
								</span>
							</div>
							<div>
								<span class="text-xs text-neutral-400 font-semibold uppercase tracking-wider block">Shift</span>
								<span class="text-base font-bold text-[#F45310] mt-1 block">
									{#if data.activeShift}
										{data.activeShift.name} ({formatShiftTime(data.activeShift.start_time)}-{formatShiftTime(data.activeShift.end_time)})
									{:else}
										—
									{/if}
								</span>
					...
					<!-- Geolocation Error warning bar -->
					{#if locationError}
						<div class="mt-4 p-3 bg-amber-50 border border-amber-200 text-amber-800 text-xs rounded-xl flex items-center gap-2">
							<span class="size-2 rounded-full bg-amber-500 animate-pulse shrink-0"></span>
							<span>{locationError}</span>
						</div>
					{/if}

					<!-- Action Checkin / Checkout button -->
					<div class="pt-6">
						{#if data.todayAttendance && data.todayAttendance.check_out_time}
							<button
								disabled
								class="px-8 py-3.5 bg-neutral-100 text-neutral-400 text-base font-bold rounded-xl border border-neutral-200 cursor-not-allowed"
							>
								Attendance Marked
							</button>
						{:else if data.todayAttendance}
							<button
								...
								class="px-8 py-3.5 bg-[#F45310] text-white text-base font-bold rounded-xl shadow-md shadow-[#F45310]/20 hover:bg-[#d94406] hover:shadow-lg transition-all duration-200 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed border-none"
							>
		...
		{:else if activeDashboard === 'manager' && data.managerContext}
			<!-- ========================================== -->
			...
			<section class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
				...
				<div class="bg-white border border-neutral-200/80 rounded-2xl p-5 flex flex-col items-center justify-between text-center shadow-xs hover:-translate-y-0.5 transition-all duration-200">
					...
					<span class="text-3xl font-extrabold text-neutral-900 mt-3">{String(data.managerContext.metrics.regularization).padStart(2, '0')}</span>
					<span class="text-[11px] font-semibold text-neutral-400 uppercase tracking-wider mt-1.5">Regularization</span>
				</div>
