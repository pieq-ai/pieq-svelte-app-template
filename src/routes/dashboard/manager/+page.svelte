      ...
      if (res.ok) {
        ...
        const errorMsg = body.data?.error
          ? typeof body.data.error === "object"
            ? Object.values(body.data.error).join(", ")
            : body.data.error
          : "Check-in failed";
...
<div class="bg-[#FAF9F6] -mx-6 -my-6 p-6 min-h-screen space-y-6">
  ...
  {#if data.employee}
    {#if data.managerContext}
      ...
      <section
        class="bg-white border border-neutral-200/80 rounded-2xl p-5 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4"
      >
        <div class="flex flex-col gap-1">
          ...
          <p class="text-xs text-neutral-500">
            ...
            {#if data.todayAttendance}
              ...
              {:else}
                <span class="font-bold text-[#F45310]"
            ...
            {#if data.activeShift}
              <span class="font-semibold text-neutral-700"
                >{data.activeShift.name} ({formatShiftTime(
                  data.activeShift.start_time,
                )}:00 AM - {formatShiftTime(data.activeShift.end_time)}:00 PM)</span
              >
            {:else}
        ...
        <div class="flex items-center gap-8 text-sm">
          <div>
            <span class="text-xs text-neutral-400 font-semibold block"
              >Check In</span
            >
            <span class="text-sm font-bold text-neutral-700 mt-0.5 block"
              >{formatTimeOnly(data.todayAttendance?.check_in_time)}</span
            >
          </div>
          <div>
            <span class="text-xs text-neutral-400 font-semibold block"
            ...
            >
            <span class="text-sm font-bold text-neutral-700 mt-0.5 block"
              >{formatTimeOnly(data.todayAttendance?.check_out_time)}</span
            >
          </div>
