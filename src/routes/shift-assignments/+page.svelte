<script lang="ts">
	...
	let employeeError = $derived(isEmployeeTouched && !formEmployeeCuid ? 'Employee selection is required' : '');
	let shiftError = $derived(isShiftTouched && !formShiftCuid ? 'Shift selection is required' : '');
	...
	let paginatedAssignments = $derived(filteredAssignments.slice((currentPage - 1) * pageSize, currentPage * pageSize));
	let activeCount = $derived(assignmentsList.filter((a) => a.status === true).length);
	let inactiveCount = $derived(assignmentsList.filter((a) => a.status === false).length);
	...
	async function loadAssignments() {
		...
		try {
			...
			loadError = err instanceof ApiError ? err.message : 'Failed to load shift assignments.';
...
<div class="w-full space-y-6 px-1 py-0">
	<div class="flex flex-col gap-4 border-b border-border pb-5 sm:flex-row sm:items-end sm:justify-between">
		<div class="space-y-1">
			<h1 class="text-3xl font-bold tracking-tight sm:text-4xl wrap-break-word">Shift Assignments</h1>
		</div>
		<Button
			type="button"
	...
	<div class="space-y-3">
			...
			<Table>
				<TableHeader class="bg-muted">
					<TableRow>
						...
						<TableHead class="font-bold text-foreground text-[15px]">
							<Button variant="ghost" size="sm" class="-ml-2.5 h-8 font-bold text-foreground text-[15px]" onclick={() => handleSort('shift')}>
								Shift
						...
						<TableHead class="text-center font-bold text-foreground text-[15px]">
							<Button variant="ghost" size="sm" class="h-8 font-bold text-foreground text-[15px]" onclick={() => handleSort('effective_from')}>
								Effective From
						...
						<TableHead class="text-center font-bold text-foreground text-[15px]">
							<Button variant="ghost" size="sm" class="h-8 font-bold text-foreground text-[15px]" onclick={() => handleSort('effective_to')}>
								Effective To
							...
							>
								<TableCell>
									<div class="flex flex-col">
										<span class="font-semibold">{assignment.employee?.first_name} {assignment.employee?.last_name}</span>
										<span class="text-xs text-muted-foreground">{assignment.employee?.emp_code}</span>
									</div>
								</TableCell>
								<TableCell>
								...
								<TableCell class="text-center">
									<Badge variant={assignment.status === true ? 'default' : 'secondary'}>
		...
		<Pagination bind:currentPage={currentPage} pageSize={pageSize} totalItems={filteredAssignments.length} />
	</div>
</div>

<CrudModal
	open={isModalOpen}
	title={isViewOnly ? 'View Assignment Details' : (editingAssignment ? 'Edit Shift Assignment' : 'Assign Shift')}
	...
	{#snippet children({ cancel })}
		<form class="space-y-4" onsubmit={handleSaveAssignment}>
			...
			<div class="grid grid-cols-2 gap-4">
				...
				<div class="space-y-2">
					<Label for="effective_to">Effective To <span class="text-destructive">*</span></Label>
					<Input
						id="effective_to"
						name="effective_to"
						type="date"
			...
			<div class="flex items-center justify-end gap-3 pt-4 border-t border-border mt-6">
				...
				{:else}
					...
					>
						{isSubmitting ? UI_CONSTANTS.BUTTON_SAVING : (editingAssignment ? UI_CONSTANTS.BUTTON_UPDATE : UI_CONSTANTS.BUTTON_SAVE)}
					</Button>
				{/if}
			</div>
		</form>
