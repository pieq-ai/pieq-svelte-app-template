            ...
            {#each paginatedDepartments as dept (dept.cuid)}
              ...
              >
                ...
                <TableCell class="text-center">
                  <Badge
                    variant={dept.status === true ? "default" : "secondary"}
                    >{dept.status ? "Active" : "Inactive"}</Badge
                  >
                </TableCell>
    ...
    <form class="space-y-3" onsubmit={handleSaveDepartment}>
      ...
      {#if editingDept}
        <StatusDropdown
          id="dept_status"
          name="dept_status"
          value={formDeptStatus}
          onChange={(val) => (formDeptStatus = val)}
        />
      {/if}
      <div class="flex items-center justify-end gap-3 pt-4">
        ...
        <Button
          type="submit"
          class="bg-[#F45310] text-white hover:bg-[#F45310]/90"
          disabled={isSubmitting || (!!editingDept && !isDirty)}
        >
          {isSubmitting
