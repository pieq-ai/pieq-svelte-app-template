	...
	<div class="grid items-start gap-8 lg:grid-cols-3">
							...
							{#each filteredPolicies as policy (policy.uuid)}
								<TableRow class={!policy.status ? 'opacity-60' : ''}>
									...
									<TableCell class="text-right space-x-1">
										<a
											href={resolve(('/masters/leave-policies?edit=' + policy.uuid) as '/masters/leave-policies')}
		...
		<Card>
			<CardHeader>
				...
				<CardDescription>
					{#if editUuid}
						Update mapping rules, quotas, and limitations.
					{:else}
						Configure a new policy rule linking leave types to targets.
					{/if}
				</CardDescription>
			</CardHeader>
			<CardContent>
				...
				>
					...
					<!-- Leave Type Dropdown -->
					<div class="space-y-2">
						<Label for="leave_type_id" class={form && 'field' in form && form.field === 'leave_type_id' ? 'text-destructive' : ''}>Leave Type</Label>
						<select
							id="leave_type_id"
							name="leave_type_id"
					...
					<!-- Annual Quota -->
					<div class="space-y-2">
						<Label for="annual_quota" class={(form && 'field' in form && form.field === 'annual_quota') || quotaError ? 'text-destructive' : ''}>Annual Quota (Days)</Label>
						<Input
							id="annual_quota"
							name="annual_quota"
					...
					<!-- Max Per Month -->
					<div class="space-y-2">
						<Label for="max_per_month" class={(form && 'field' in form && form.field === 'max_per_month') || maxPerMonthError ? 'text-destructive' : ''}>Max Per Month (Optional)</Label>
						<Input
							id="max_per_month"
							name="max_per_month"
					...
					<div class="flex items-center space-x-2 pt-2">
						<input type="checkbox" id="carry_forward_allowed" name="carry_forward_allowed" bind:checked={carryForwardAllowed} class="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary" />
						<Label for="carry_forward_allowed" class="cursor-pointer select-none">Carry Forward Allowed</Label>
					</div>

					{#if carryForwardAllowed}
						<div transition:slide class="space-y-2 pl-4">
							<Label for="max_carry_forward_days" class={(form && 'field' in form && form.field === 'max_carry_forward_days') || carryForwardDaysError ? 'text-destructive' : ''}>Max Carry Forward Days</Label>
							<Input
					...
					<!-- Min Service Days -->
					<div class="space-y-2">
						<Label for="min_service_days" class={(form && 'field' in form && form.field === 'min_service_days') || minServiceDaysError ? 'text-destructive' : ''}>Min Service Days (Active service req.)</Label>
						<Input
							id="min_service_days"
					...
					<!-- Other Checkboxes -->
					<div class="flex items-center space-x-2">
						<input type="checkbox" id="allow_half_day" name="allow_half_day" bind:checked={allowHalfDay} class="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary" />
						<Label for="allow_half_day" class="cursor-pointer select-none">Allow Half Day</Label>
					</div>

					<div class="flex items-center space-x-2">
						<input type="checkbox" id="requires_document" name="requires_document" bind:checked={requiresDocument} class="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary" />
						<Label for="requires_document" class="cursor-pointer select-none">Requires Document Attachment</Label>
					</div>

					<!-- Gender Specific Rules -->
					<div class="flex items-center space-x-2">
						<input type="checkbox" id="gender_specific" name="gender_specific" bind:checked={genderSpecific} class="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary" />
						<Label for="gender_specific" class="cursor-pointer select-none">Gender Specific Leave</Label>
					</div>

					{#if genderSpecific}
						<div transition:slide class="space-y-2 pl-4">
							<Label for="applicable_gender" class={(form && 'field' in form && form.field === 'applicable_gender') || genderError ? 'text-destructive' : ''}>Applicable Gender</Label>
							<select
					...
					<div class="space-y-2">
						...
						{#if editUuid}
							<a
								href={resolve('/masters/leave-policies')}
