      ...
      <Table>
        <TableHeader class="bg-muted">
          <TableRow>
            ...
            <TableHead
              class="text-center font-bold text-foreground text-[15px] whitespace-nowrap"
            >
              <Button
                variant="ghost"
                size="sm"
            ...
            {#each paginatedDesignations as designation (designation.cuid)}
              ...
              >
                ...
                <TableCell class="text-center">
                  <Badge
                    variant={designation.status === true
                      ? "default"
                      : "secondary"}
                    >{designation.status ? "Active" : "Inactive"}</Badge
                  >
                </TableCell>
    ...
    <form class="space-y-3" onsubmit={handleSaveDesignation}>
      <div class="space-y-2">
        ...
        {#if nameValidationError || backendError}
          ...
          >
            {nameValidationError || backendError}
          </p>
        {/if}
      </div>
      {#if editingDesignation}
        <StatusDropdown
          ...
          value={formDesignationStatus}
          onChange={(val) => (formDesignationStatus = val)}
        />
      {/if}
      <div class="flex items-center justify-end gap-3 pt-4">
        ...
        <Button
          type="submit"
          class="bg-[#F45310] text-white hover:bg-[#F45310]/90"
          disabled={isSubmitting || (!!editingDesignation && !isDirty)}
        >
          {isSubmitting
