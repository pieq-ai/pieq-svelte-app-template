...
export async function GET({ locals, params }: RequestEvent) {
  try {
    ...
    if (!cuid) {
      return json(
        { success: false, message: "Invalid salary structure ID" },
        { status: 400 },
      );
    }
    ...
    return json(
      {
        success: false,
        message:
          (error as Error).message || "Failed to retrieve salary structure",
      },
...
export async function PUT({ locals, params, request }: RequestEvent) {
  try {
    ...
    return json(
      {
        message:
          (error as Error).message || "Failed to update salary structure",
      },
...
export async function DELETE({ locals, params }: RequestEvent) {
  try {
    ...
    console.error(
      `Error in DELETE /api/salary-structures/${params.cuid}:`,
      error,
    );
    const isNotFound = (error as Error).name === "SalaryStructureNotFoundError";
    return json(
      {
        success: false,
        message:
          (error as Error).message || "Failed to deactivate salary structure",
      },
      { status: isNotFound ? 404 : 500 },
    );
  }
...
export async function POST({ locals, params, request }: RequestEvent) {
  try {
    ...
    return json(
      { data: { cuid: revision.cuid, message: "success" } },
      { status: 201 },
    );
  } catch (error) {
    console.error(
      `Error in POST /api/salary-structures/${params.cuid}:`,
      error,
    );
    const isNotFound = (error as Error).name === "SalaryStructureNotFoundError";
    const isValidationError =
