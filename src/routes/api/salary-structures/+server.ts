    ...
    return json(
      {
        success: false,
        message:
          (error as Error).message || "Failed to retrieve salary structures",
      },
      { status: 500 },
    );
  }
...
export async function POST({ locals, request }: RequestEvent) {
  try {
    ...
    return json(
      { data: { cuid: created.cuid, message: "success" } },
      { status: 201 },
    );
  } catch (error) {
    ...
    return json(
      {
        message:
          (error as Error).message || "Failed to create salary structure",
      },
