...
import type {
  ParsedPayrollRow,
  ParseResult,
} from "$lib/server/utils/excel-parser.js";
...
export async function POST({ locals, request }) {
  try {
    ...
    const defaultMonth = defaultMonthRaw
      ? parseInt(String(defaultMonthRaw), 10)
      : null;
    const defaultYear = defaultYearRaw
      ? parseInt(String(defaultYearRaw), 10)
      : null;

    if (!defaultMonth || defaultMonth < 1 || defaultMonth > 12) {
      return json(
        { message: "A valid pay month (1-12) is required." },
        { status: 400 },
      );
    }
    if (!defaultYear || defaultYear < 2000 || defaultYear > 9999) {
      return json(
        { message: "A valid 4-digit pay year is required." },
        { status: 400 },
      );
    }
    ...
    return json(
      {
        message:
          (error as Error).message || "Failed to process payroll upload.",
      },
      { status: 500 },
    );
  }
