...
export class KeycloakService {
        ...
        switch (response.status) {
            ...
            default:
                if (response.status >= 500) {
                    throw new KeycloakUnavailableError(response.status, 'Keycloak service unavailable', details);
                }
                throw new KeycloakError(response.status, `Keycloak API error: ${response.statusText}`, details);
        }
    }
    ...
    private async fetchApi(path: string, options: RequestInit = {}): Promise<Response> {
        ...
        logger.info('Keycloak request started', { endpoint: path, method: options.method || 'GET', requestId });
        ...
        try {
            ...
            throw new KeycloakUnavailableError(500, 'Failed to connect to Keycloak', (error as Error).message);
        }
    }
     ...
     */
    private async fetchAndParse(path: string, options: RequestInit = {}): Promise<any> {
    ...
    public async getAdminAccessToken(): Promise<string> {
        ...
        const data: KeycloakTokenResponse = await this.handleResponse(response, path);
    ...
    public async createUser(payload: CreateUserPayload): Promise<{ keycloakSub: string }> {
        ...
        if (!location) {
            throw new KeycloakError(500, 'User created but Location header is missing');
        }
    ...
    public async searchUsers(params: { username?: string; email?: string; search?: string }): Promise<KeycloakUser[]> {
        ...
        return this.fetchAndParse(`/admin/realms/${this.config.realm}/users${queryString}`);
    }
    ...
    public async resetPassword(id: string, password: string, temporary: boolean = true): Promise<void> {
    ...
    public async triggerRequiredActions(id: string, actions: string[], options?: RequiredActionOptions): Promise<void> {
        ...
        if (options?.redirect_uri) params.append('redirect_uri', options.redirect_uri);
        if (options?.lifespan) params.append('lifespan', options.lifespan.toString());
    ...
    public async getRealmRole(roleName: string): Promise<KeycloakRole> {
        return this.fetchAndParse(`/admin/realms/${this.config.realm}/roles/${roleName}`);
    }

    public async assignRealmRole(userId: string, role: KeycloakRole): Promise<void> {
        await this.fetchAndParse(`/admin/realms/${this.config.realm}/users/${userId}/role-mappings/realm`, {
            method: 'POST',
            headers: new Headers({ 'Content-Type': 'application/json' }),
            body: JSON.stringify([
                {
    ...
    public async removeRealmRole(userId: string, role: KeycloakRole): Promise<void> {
        await this.fetchAndParse(`/admin/realms/${this.config.realm}/users/${userId}/role-mappings/realm`, {
            method: 'DELETE',
            headers: new Headers({ 'Content-Type': 'application/json' }),
            body: JSON.stringify([
    ...
    public async addUserToGroup(userId: string, groupId: string): Promise<void> {
        await this.fetchAndParse(`/admin/realms/${this.config.realm}/users/${userId}/groups/${groupId}`, {
            method: 'PUT'
        });
    }
}
