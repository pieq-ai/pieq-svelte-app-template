...
describe('Master Data Service', () => {
	...
	describe('getMasterData', () => {
		...
		it('should filter by search query', async () => {
			const mockData = [
				{ id: 1, cuid2: 'l1', languages_name: 'English' },
				{ id: 2, cuid2: 'l2', languages_name: 'Spanish' }
			];
			vi.mocked(masterDataDao.list).mockResolvedValue(mockData as any);
		...
		it('should filter states by country_cuid2', async () => {
			const mockData = [
				{ id: 1, cuid2: 's1', state_name: 'California', country_cuid2: 'c1' },
				{ id: 2, cuid2: 's2', state_name: 'Texas', country_cuid2: 'c1' },
				{ id: 3, cuid2: 's3', state_name: 'Ontario', country_cuid2: 'c2' }
			];
			vi.mocked(masterDataDao.list).mockResolvedValue(mockData as any);
	...
	describe('createMasterData', () => {
		...
		describe('state creation', () => {
			...
			it('should throw if country is not found', async () => {
				vi.mocked(masterDataDao.findByCuid2).mockResolvedValue(null);
				await expect(masterDataService.createMasterData('states', { name: 'California', country_cuid2: 'invalid' })).rejects.toThrow('Country not found');
			});
		});

		it('should throw if entry already exists (duplicate name)', async () => {
			vi.mocked(masterDataDao.list).mockResolvedValue([{ cuid2: 'bg1', blood_group_name: 'O+' }] as any);
		...
		it('should allow duplicate name in states if country is different', async () => {
			vi.mocked(masterDataDao.list).mockResolvedValue([
				{ cuid2: 's1', state_name: 'Georgia', country_cuid2: 'c1' }
			] as any);
			vi.mocked(masterDataDao.findByCuid2).mockResolvedValue({ id: 1 } as any);
			vi.mocked(masterDataDao.create).mockResolvedValue({ cuid2: 's2', state_name: 'Georgia' } as any);

			await expect(masterDataService.createMasterData('states', { name: 'Georgia', country_cuid2: 'c2' })).resolves.toBeDefined();
		});
	...
	describe('updateMasterData', () => {
		...
		it('should throw if state country is missing during update', async () => {
			vi.mocked(masterDataDao.findByCuid2).mockResolvedValue({ id: 1, cuid2: 's1', state_name: 'Texas' } as any);
		...
		it('should throw if duplicate name exists on another record', async () => {
			vi.mocked(masterDataDao.findByCuid2).mockResolvedValue({ id: 1, cuid2: 'bg1', blood_group_name: 'A+' } as any); // current record
			vi.mocked(masterDataDao.list).mockResolvedValue([{ cuid2: 'bg2', blood_group_name: 'O+' }] as any); // other record
