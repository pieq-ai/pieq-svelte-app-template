<script lang="ts">
	...
	let activePlaceholder = $derived(placeholder || `Select ${label.replace(/[*]/g, '').trim()}`);
	...
	let selectedCount = $derived(multiple ? (Array.isArray(value) ? value.length : (value ? 1 : 0)) : 0);
				...
				<ScrollArea class="max-h-60 overflow-y-auto">
					<div class="p-1">
						{#each filteredOptions as option (option.id)}
							<div class="flex items-center justify-between rounded-sm px-2 py-1.5 text-sm hover:bg-accent hover:text-accent-foreground group">
								<button
									type="button"
								...
								>
									{#if multiple}
										<div class="pointer-events-none flex items-center">
											<Checkbox checked={isSelected(option.id)} tabindex={-1} />
										</div>
									{:else}
						...
						{:else}
							<div class="flex items-center justify-center py-8 text-sm text-muted-foreground text-center min-h-[100px]">
