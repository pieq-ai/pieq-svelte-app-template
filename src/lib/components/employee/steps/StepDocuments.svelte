  ...
  {#if mode !== "view"}
    <div class="flex justify-end">
      <Button
        ...
        onclick={addDocument}
        disabled={isSubmitting || readingCount > 0}
      >
        Add Document
      </Button>
    </div>
  ...
  <div class="space-y-4">
    {#each documents as doc (doc._id)}
      {#if doc.file_name && !doc.isReplacing}
        ...
        <div
          class="rounded-xl border border-border p-5 relative shadow-sm bg-card hover:shadow-md transition-shadow"
        >
          <div
            class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-2"
          >
            <div class="flex items-start gap-4 w-full">
              <div
                class="flex items-center justify-center size-10 rounded-lg bg-hrms-primary/10 text-hrms-primary shrink-0 mt-1"
              >
                <svg
              ...
              <div class="space-y-3 w-full max-w-sm">
                ...
                <p
                  class="text-sm font-medium text-muted-foreground truncate w-full"
                  title={doc.file_name}
                >
                  {doc.file_name}
                </p>
            ...
            <div
              class="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-green-100 text-green-700 text-xs font-semibold whitespace-nowrap self-start"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="14"
        ...
        <div class="rounded-lg border border-border p-4 relative bg-muted/20">
          <div class="flex justify-between items-center mb-4">
            <h4 class="text-sm font-semibold">
              {doc.isReplacing ? "Replace Document" : "New Document"}
            </h4>
            {#if mode !== "view"}
              <div class="flex items-center gap-2">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  class="h-7 px-3 text-destructive hover:bg-destructive/10"
                  onclick={() => {
                    if (doc.isReplacing) {
                      // Restore from scalar backup — no File objects needed
                      doc.document_type_cuid =
                        doc._backupDocumentTypeCuid ?? doc.document_type_cuid;
                      ...
                      doc.document_base64 =
                        doc._backupBase64 ?? doc.document_base64;
            ...
            <div class="space-y-2">
              {#if mode !== "view"}
                ...
                <Input
                  ...
                  class={isTouched && validateRequired(doc.file_name)
