...
describe('API Endpoint Integration Tests', () => {
  ...
  describe('Roles API Endpoints', () => {
    it('should test full HTTP lifecycles and error scenarios for Roles', async () => {
      ...
      const res415 = await rolesApi.POST({ request: badHeadersRequest, locals: { user: { permissions: ['role:view'] } } } as any);
      ...
      const res400BadName = await rolesApi.POST({ request: badNameRequest, locals: { user: { permissions: ['role:view'] } } } as any);
      ...
      const res400BadKeys = await rolesApi.POST({ request: badKeysRequest, locals: { user: { permissions: ['role:view'] } } } as any);
      ...
      const res201 = await rolesApi.POST({ request: validPostRequest, locals: { user: { permissions: ['role:view'] } } } as any);
      ...
      const res409 = await rolesApi.POST({ request: duplicateRequest, locals: { user: { permissions: ['role:view'] } } } as any);
      expect(res409.status).toBe(409);

      // 6. GET: 200 OK
      const getUrl = new URL('http://localhost/api/roles?includeInactive=true');
      const getRes = await rolesApi.GET({ url: getUrl, locals: { user: { permissions: ['role:view'] } } } as any);
      ...
      const putRes400 = await rolesCuidApi.PUT({ request: invalidCuidPut, params: { cuid: 'short' }, locals: { user: { permissions: ['role:view'] } } } as any);
      ...
      const putRes404 = await rolesCuidApi.PUT({ request: notFoundPut, params: { cuid: nonExistentCuid }, locals: { user: { permissions: ['role:view'] } } } as any);
      ...
      const putRes200 = await rolesCuidApi.PUT({ request: validPutRequest, params: { cuid: cuid }, locals: { user: { permissions: ['role:view'] } } } as any);
      expect(putRes200.status).toBe(200);

      // 10. DELETE: 200 OK - Soft-delete
      const deleteRes = await rolesCuidApi.DELETE({ params: { cuid: cuid }, locals: { user: { permissions: ['role:view'] } } } as any);
  ...
  describe('Shifts API Endpoints', () => {
    it('should test full HTTP lifecycles and error scenarios for Shifts', async () => {
      ...
      const malformedRes = await shiftsApi.POST({ request: malformedRequest, locals: { user: { permissions: ['shift:view'] } } } as any);
      ...
      const res201 = await shiftsApi.POST({ request: validPostRequest, locals: { user: { permissions: ['shift:view'] } } } as any);
      expect(res201.status).toBe(201);
      const postJson = await res201.json();
      const cuid = postJson.data.cuid;
      expect(cuid).toBeDefined();
      ...
      // 3. GET: 200 OK
      const getRes = await shiftsApi.GET({ url: new URL('http://localhost/api/shifts'), locals: { user: { permissions: ['shift:view'] } } } as any);
      expect(getRes.status).toBe(200);
      ...
      const putRes = await shiftsCuidApi.PUT({ request: validPutRequest, params: { cuid: cuid }, locals: { user: { permissions: ['shift:view'] } } } as any);
      expect(putRes.status).toBe(200);

      // 5. DELETE: 200 OK - Soft-delete
      const deleteRes = await shiftsCuidApi.DELETE({ params: { cuid: cuid }, locals: { user: { permissions: ['shift:view'] } } } as any);
      expect(deleteRes.status).toBe(200);

      // 6. PATCH: 200 OK - Activation
      const patchRes = await shiftsCuidApi.PATCH({ params: { cuid: cuid }, locals: { user: { permissions: ['shift:view'] } } } as any);
  ...
  describe('Locations API Endpoints', () => {
    it('should test full HTTP lifecycles and error scenarios for Company Locations', async () => {
      ...
      const res201 = await locationsApi.POST({ request: validPostRequest, locals: { user: { permissions: ['location:view'] } } } as any);
      expect(res201.status).toBe(201);
      const postJson = await res201.json();
      const cuid = postJson.data.cuid;
      expect(cuid).toBeDefined();
      ...
      // 2. GET: 200 OK
      const getRes = await locationsApi.GET({ url: new URL('http://localhost/api/organization_location'), locals: { user: { permissions: ['location:view'] } } } as any);
      expect(getRes.status).toBe(200);
      ...
      const putRes = await locationsCuidApi.PUT({ request: validPutRequest, params: { cuid: cuid }, locals: { user: { permissions: ['location:view'] } } } as any);
      expect(putRes.status).toBe(200);

      // 4. DELETE: 200 OK - Soft-delete
      const deleteRes = await locationsCuidApi.DELETE({ params: { cuid: cuid }, locals: { user: { permissions: ['location:view'] } } } as any);
      expect(deleteRes.status).toBe(200);

      // 5. PATCH: 200 OK - Activation
      const patchRes = await locationsCuidApi.PATCH({ params: { cuid: cuid }, locals: { user: { permissions: ['location:view'] } } } as any);
      expect(patchRes.status).toBe(200);
