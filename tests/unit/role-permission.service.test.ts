...
describe('Role Permission Service', () => {
	...
	describe('getRolePermissionMatrix', () => {
		it('should retrieve and format matrix data correctly', async () => {
			vi.mocked(systemRoleDao.list).mockResolvedValue([
				{ id: 1, cuid: 'role1', name: 'Admin', status: true, created_at: new Date('2026-05-29T12:00:00Z'), created_by: null, updated_at: new Date('2026-05-29T12:00:00Z'), updated_by: null }
			] as any);
	...
	describe('assignPermissionsToRole', () => {
		...
		it('should create new mappings and skip existing ones', async () => {
			...
			vi.mocked(rolePermissionDao.findByRoleAndPermission).mockImplementation(async (roleId: any, permId: any) => {
				if (permId === 'perm1') return { id: 1, system_role_cuid: roleId, permission_cuid: permId } as any;
				return null;
			});

			vi.mocked(rolePermissionDao.create).mockImplementation(async (data: any) => ({ id: 2, ...data } as any));
