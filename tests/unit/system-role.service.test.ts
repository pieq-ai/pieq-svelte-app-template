...
describe('System Role Service', () => {
	...
	describe('getSystemRoles', () => {
		it('should return mapped public system roles', async () => {
			const mockData = [
				{ id: 1, cuid: 'abc', name: 'Admin', status: true, created_at: new Date('2026-05-29T12:00:00Z'), created_by: null, updated_at: new Date('2026-05-29T12:00:00Z'), updated_by: null },
				{ id: 2, cuid: 'xyz', name: 'Editor', status: false, created_at: new Date('2026-05-29T12:00:00Z'), created_by: null, updated_at: new Date('2026-05-29T12:00:00Z'), updated_by: null }
			];
	...
	describe('getSystemRoleById', () => {
		...
		it('should return mapped system role if found', async () => {
			const mockData = { id: 1, cuid: 'abc', name: 'Admin', status: true, created_at: new Date('2026-05-29T12:00:00Z'), created_by: null, updated_at: new Date('2026-05-29T12:00:00Z'), updated_by: null };
	...
	describe('createSystemRole', () => {
		...
		it('should throw an error if system role already exists', async () => {
			vi.mocked(systemRoleDao.list).mockResolvedValue([{ id: 1, name: 'Admin' } as any]);
		...
		it('should create and return the new system role', async () => {
			...
			const mockCreated = { id: 2, cuid: 'new123', name: 'Editor', status: true, created_at: new Date('2026-05-29T12:00:00Z'), created_by: null, updated_at: new Date('2026-05-29T12:00:00Z'), updated_by: null };
	...
	describe('updateSystemRole', () => {
		...
		it('should throw error if new system role already exists on another record', async () => {
			vi.mocked(systemRoleDao.findByCuid2).mockResolvedValue({ id: 1, cuid: 'abc', name: 'Old Role' } as any);
			vi.mocked(systemRoleDao.list).mockResolvedValue([{ id: 2, cuid: 'xyz', name: 'New Role' } as any]);
		...
		it('should allow updating with same name', async () => {
			vi.mocked(systemRoleDao.findByCuid2).mockResolvedValue({ id: 1, cuid: 'abc', name: 'Same Role' } as any);
			vi.mocked(systemRoleDao.list).mockResolvedValue([{ id: 1, cuid: 'abc', name: 'Same Role' } as any]);
			vi.mocked(systemRoleDao.update).mockResolvedValue({ id: 1, cuid: 'abc', name: 'Same Role', status: true, created_at: new Date('2026-05-29T12:00:00Z'), created_by: null, updated_at: new Date('2026-05-29T12:00:00Z'), updated_by: null } as any);

			await systemRoleService.updateSystemRole('abc', { name: 'Same Role' });
			expect(systemRoleDao.update).toHaveBeenCalledWith(1, { name: 'Same Role' });
		});

		it('should update system role fields correctly', async () => {
			vi.mocked(systemRoleDao.findByCuid2).mockResolvedValue({ id: 1, cuid: 'abc', name: 'Old Role' } as any);
			vi.mocked(systemRoleDao.list).mockResolvedValue([]);
			vi.mocked(systemRoleDao.update).mockResolvedValue({ id: 1, cuid: 'abc', name: 'New Role', status: false, created_at: new Date('2026-05-29T12:00:00Z'), created_by: null, updated_at: new Date('2026-05-29T12:00:00Z'), updated_by: null } as any);

			const result = await systemRoleService.updateSystemRole('abc', { name: 'New Role', status: false });

			expect(systemRoleDao.update).toHaveBeenCalledWith(1, { name: 'New Role', status: false });
	...
	describe('deleteSystemRole', () => {
		...
		it('should perform a soft delete by setting status to false', async () => {
			vi.mocked(systemRoleDao.findByCuid2).mockResolvedValue({ id: 1, cuid: 'abc', name: 'Admin' } as any);
			vi.mocked(systemRoleDao.update).mockResolvedValue({ id: 1, cuid: 'abc', name: 'Admin', status: false, created_at: new Date('2026-05-29T12:00:00Z'), created_by: null, updated_at: new Date('2026-05-29T12:00:00Z'), updated_by: null } as any);
